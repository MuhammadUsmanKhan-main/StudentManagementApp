...
let SectionService = class SectionService {
    ...
    async updateSection(id, updateSectionDto) {
        ...
        if (updateSectionDto.name &&
            ...
            const existing = await this.findSection(updateSectionDto.name, updateSectionDto.courseId);
            if (existing) {
                throw new common_1.ConflictException("Another section with same name exists in this course");
            }
        }
    ...
    async deleteSection(id) {
        ...
        try {
            ...
            if (error.code === 'P2003') {
                throw new common_1.ConflictException("Cannot delete section as it is associated with other records.");
            }
